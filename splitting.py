...
class MONDouglasRachford(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            while (err > self.tol and it < self.max_iter):
                u_12 = tuple(2 * z[i] - u[i] for i in range(n))
                z_12 = self.linear_module.inverse(*tuple(u_12[i] + self.alpha * bias[i] for i in range(n)))
                u_ = tuple(2 * z_12[i] - u_12[i] for i in range(n))
                u = tuple(0.5 * (u_[i] + u[i]) for i in range(n))
                # u = tuple(1.2 * u_[i] - 0.2 * u[i] for i in range(n))
                zn = self.nonlin_module(*u)
...
class MONProximalDecomposition(nn.Module):
    ...
    def forward(self, x):
        ...
        self.linear_module.init_inverse(1 + self.alpha, -self.alpha)
        with torch.no_grad():
            x1 = tuple(torch.zeros(s, dtype=x.dtype, device=x.device)
                      for s in self.linear_module.z_shape(x.shape[0]))
            y1 = tuple(torch.zeros(s, dtype=x.dtype, device=x.device)
                      for s in self.linear_module.z_shape(x.shape[0]))
            x2 = tuple(torch.zeros(s, dtype=x.dtype, device=x.device)
                      for s in self.linear_module.z_shape(x.shape[0]))
            y2 = tuple(torch.zeros(s, dtype=x.dtype, device=x.device)
                      for s in self.linear_module.z_shape(x.shape[0]))
            z = tuple(torch.zeros(s, dtype=x.dtype, device=x.device)
                      for s in self.linear_module.z_shape(x.shape[0]))
            n = len(x1)
            ...
            errs = []
            while (err > self.tol and it < self.max_iter):
                x_112 = self.linear_module.inverse(*tuple(x1[i] + y1[i] + self.alpha * bias[i] for i in range(n)))
                y_112 = tuple(x1[i] - x_112[i] + y1[i] for i in range(n))
                x_212 = self.nonlin_module(*tuple(x2[i] + y2[i] for i in range(n)))
                y_212 = tuple(x2[i] - x_212[i] + y2[i] for i in range(n))

                x1 = x2 = tuple(0.5 * (x_112[i] + x_212[i]) for i in range(n))
                y1 = tuple(0.5 * (y_112[i] - y_212[i]) for i in range(n))
                y2 = tuple(-y1[i] for i in range(n))
